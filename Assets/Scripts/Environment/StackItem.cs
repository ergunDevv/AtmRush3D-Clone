...
public class StackItem : MonoBehaviour
{
    ...
    private void Start()
    {
        PlayerPrefs.GetInt("quantityStartItem");
    }
    private void Follow()
    {
    ...
    public void CreateWood(int quantity_length)
    {

        for (int i = 0; i < quantity_length; i++)
        {
            Instantiate(StackManager.Instance.wood_Prefab, StackManager.Instance.stackRoot.position, Quaternion.identity);
        }
